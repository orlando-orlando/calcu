...
export function drenCanal(flujoMaximo, tipoDrenCanal, datos) {
  ...
  if (distanciaCMDC > 0) {
    ...
    const cargaTotalCMDC = cargaTramoCMDC + cargaCodoCMDC;

    tablaDistanciaCMDC = {
      distanciaCMDC: fix2(distanciaCMDC),
      flujoCMDC: fix2(flujoCMDC),
    ...
    if (!resumenTramosDC[diametroCMDC])
      resumenTramosDC[diametroCMDC] = { tuberia_m: 0, tees: 0, codos: 0, reducciones: 0 };
  ...
  for (let i = 0; i < numDrenCanalFinal; i++) {
    ...
    else resumenTramosDC[diametroSeleccionado].codos += 1;
  ...
  const cargaCMDC = tablaDistanciaCMDC ? parseFloat(tablaDistanciaCMDC.cargaTotalCMDC) : 0;
  const sumaFinal = sumaCargaTramos + cargaCMDC;
