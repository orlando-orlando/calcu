...
export default function useCalculosHidraulicos(datos, fix2) {
    const retorno = useCallback((flujoMaximo, tipoRetorno) => {
        ...
        const cargaDisparoTramo = (longitudDisparo / 0.3048 * cargaDisparoBase) / 100;
        ...
        for (let i = 0; i < numRetornos; i++) {
            ...
            // --- Accesorios y cálculos ---
            const tipoAccesorio = (i === numRetornos - 1) ? "codo" : "tee";
            let longEqTeeRow = tipoAccesorio === "tee" ? teeLinea[diametroSeleccionado] : 0;
            let cargaTeeRow = (longEqTeeRow * cargaSeleccionada) / 100;

            let longEqCodoUnit = codo[diametroSeleccionado];
            ...
            if (diametroAnterior && diametroAnterior !== diametroSeleccionado) {
                longitudEqReduccion = reduccion[diametroSeleccionado];
        ...
        const sumaFinal = sumaCargaTramos + cargaDisparoTotal + (tablaDistanciaCM.cargaTotalCM || 0);
